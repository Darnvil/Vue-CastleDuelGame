...
Vue.component('cloud',
{
	...
	methods:
	{
		...
		startAnimation(delay = 0)
		{
			...
			const animDuration = Math.random() * (max - min) + min;

			this.style.zIndex = Math.round(max * animDuration);

			const top = Math.random()	* ((window.innerHeight + 600) * 0.3);

			new TWEEN.Tween({ value: -width })
			.to({ value: window.innerWidth }, animDuration)
			.delay(delay)
			.onUpdate(function ()
			{
			...
			.onComplete(() =>
			{
				this.startAnimation(Math.random() * min);
			})
			.start();
		}
	},
