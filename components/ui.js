Vue.component('top-bar',
{
	...
	template:
	`<div class="top-bar" :class="'player-' +
	currentPlayerIndex" >
...
Vue.component('overlay-content-player-turn',
{
	...
	`<div>
		<div class="big" v-if="player.skipTurn">
			{{player.name}}, <br>your turn has skipped! 
		</div>
		<div class="big" v-else>
			{{player.name}}, <br>your turn!
		</div>
...
Vue.component('overlay-content-last-play',
{
	...
	`<div>
		<div v-if="opponent.skippedTurn">
			{{opponent.name}} last turn was skipped!
    </div>
		<template v-else>
			<div>
...
Vue.component('player-result',
{
	...
	computed:
	{
		result()
		{
			return this.player.dead ? 'defeated' : 'victorius';
		}
	}
});
